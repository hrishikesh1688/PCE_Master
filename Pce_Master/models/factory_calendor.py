    ...
    def get_year_fact(self):
        ...
        for hline in holiday:
            ...
            for date in date_generated:
                ...
                self._cr.execute("select id,year,weak_no,ydate,shift from factory_calendor where ydate=%s",(sdate3,))
                temp= self.env.cr.fetchall()
                for l in temp:
                    if sdate3==l[3]:
                        self.env.cr.execute("update factory_calendor set flag=%s where ydate=%s",('0',sdate3,))
                    else:
                        print('Record Found Not')
        ##Update the sequence the factory_calendor
        self._cr.execute("select flag,ydate from factory_calendor where year=%s order by ydate",(self.year,))
        temp1= self.env.cr.fetchall()
        t1=1
        for l1 in temp1:
            if str(l1[0])=='0':
                self.env.cr.execute("update factory_calendor set seq_no=%s where ydate=%s",('0',l1[1],))
            else:
                self.env.cr.execute("update factory_calendor set seq_no=%s where ydate=%s",(t1,l1[1],))
                t1=t1+1